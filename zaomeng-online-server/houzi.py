...
class Houzi():
        ...
        if stairs.moving:
            ...
            if self.attacked_time <= 0:
                ...
                if self.backward and (self.rect.left > 0):
                    ...
                    self.bearing = True
                    self.move += 1
                    if self.move % 4 and self.move % 4 < 2:
                        self.image = self.img_jingzhi_left
                    elif self.move % 4 > 2:
    ...
    def jump_control(self, stairs):
        ...
        if (self.rect.bottom >= self.jump_position - self.jump_limit) and self.jump_up:
            self.rect.bottom -= 18 - round(self.jump_step * (self.jump_position - self.rect.bottom))
            if self.bearing == False:
        ...
        # 跳跃_下降
            ...
            self.go_down(stairs)
            # 下降到台阶上?
            if (self.rect.bottom < stairs.rect.centery) and (self.rect.bottom > stairs.rect.top + 45) and (
                    self.rect.centerx >= stairs.rect.left) and (self.rect.centerx <= stairs.rect.right):
                self.jump = False
                self.jump_down = False
    ...
    def go_down(self, stairs):
        ...
        if self.down:
            # 从梯子上下去
            self.rect.bottom += 14 - round(0.022222 * (self.jump_position - self.rect.bottom))
        else:
            self.rect.bottom += 18 - round(self.jump_step * (self.jump_position - self.rect.bottom))
        if self.bearing == False:
